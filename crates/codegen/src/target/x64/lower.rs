...
use crate::{
    ...
    lir::{
        DefOperand, DefOperandConstraint, OperandPos, PhysReg, PhysRegSet, RegBank, UseOperand,
        VirtReg,
    },
...
use super::{
    ...
    FullOperandSize, OperandSize, RB_GPR, RB_XMM, REG_R8, REG_R9, REG_RAX, REG_RCX, REG_RDI,
...
impl MachineLower for X64Machine {
    fn reg_bank_for_type(&self, ty: Type) -> RegBank {
        match ty {
            Type::I32 | Type::I64 | Type::Ptr => RB_GPR,
            Type::F64 => RB_XMM,
        }
    }
            ...
            NodeKind::Sfill(width) => {
                ...
                match (ty, width) {
                    ...
                    _ => {
                        ...
                        let input = ctx.get_value_vreg(input);
                        let output = ctx.get_value_vreg(output);
                        let temp = ctx.create_temp_vreg(RB_GPR);
...
fn select_fconst64(
    ...
    if val.bits() == 0 {
        ...
        match machine.config.internal_code_model {
            ...
            CodeModel::LargeAbs => {
                let addr = ctx.create_temp_vreg(RB_GPR);
                ctx.emit_instr(
...
fn select_uinttofloat(ctx: &mut IselContext<'_, '_, X64Machine>, node: Node) {
    ...
    match input_ty {
        Type::I32 => {
            let tmp = ctx.create_temp_vreg(RB_GPR);
            ctx.emit_instr(
        ...
        Type::I64 => {
            let tmp_gpr1 = ctx.create_temp_vreg(RB_GPR);
            let tmp_gpr2 = ctx.create_temp_vreg(RB_GPR);
...
fn select_floattouint(machine: &X64Machine, ctx: &mut IselContext<'_, '_, X64Machine>, node: Node) {
    ...
    match output_ty {
        ...
        Type::I64 => {
            ...
            let tmp_xmm1 = ctx.create_temp_vreg(RB_XMM);
            let tmp_xmm2 = ctx.create_temp_vreg(RB_XMM);
...
fn emit_call_abs(ctx: &mut IselContext<'_, '_, X64Machine>, node: Node, func: FunctionRef) {
    emit_call_wrapper(ctx, node, ctx.node_inputs(node), |ctx, retvals, args| {
        let target = ctx.create_temp_vreg(RB_GPR);
        ctx.emit_instr(
            X64Instr::FuncAddrAbs(func),
...
fn emit_udiv(
    ...
    let rdx_in = ctx.create_temp_vreg(RB_GPR);
    emit_mov_rz(ctx, rdx_in);

    let rax_out = ctx.create_temp_vreg(RB_GPR);
    let rdx_out = ctx.create_temp_vreg(RB_GPR);
    ctx.emit_instr(
...
fn emit_fpu_cmp_sequence(
    ...
    let tmp_xmm_out = ctx.create_temp_vreg(RB_XMM);
    let tmp_gpr_out = ctx.create_temp_vreg(RB_GPR);
    ctx.emit_instr(
